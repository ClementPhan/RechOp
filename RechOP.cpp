...
/*calcule du score d'un groupe*/
int score(groupe& a, vector<vector<int>> A, vector<vector<int>> fournisseur, int depot) {
	if (a.nombre_de_fournisseurs == 1) {
		return(score_groupe(a, A, depot))
	}
	else {
		int y, x = score_groupe(a, A, depot);   // Optimisation euristique sur l'ordre des villes dans le groupe 
		for (int i = 0; i < 100; i++) {
			int p = rand() % a.nombre_de_fournisseurs;
			int l = rand() % a.nombre_de_fournisseurs;
			swap(a.fournisseurs[p], a.fournisseurs[l]);
			y = score_groupe(a, A, depot);
			if (y > x) {
				swap(a.fournisseurs[p], a.fournisseurs[l]);
			}
			else {
		...
		int penal = 100;                           // Penalisation pour que la dernière tournée de camion ne soit pas presque vide
		int reste = 0;
		int p = 0;        
		for (int j = 1; j < 7; j++) {
			for (int i = 0; i < a.nombre_de_fournisseurs; i++) {
				reste += fournisseur[a.fournisseurs[i]][j];
			}
			p += 100 * penal % 25;
		}	
		return(x+p);
...
/*Calcule du score des groupes ordonné d'une certaine manière*/
int score_groupe(groupe a,vector<vector<int> > A,int depot){   
    int co=0;
	for (int i = 0; i < a.nombre_de_fournisseurs - 1; i++) {
		for (int j = 1; j < a.nombre_de_fournisseurs; j++) {
			co = co + A[a.fournisseurs[i]][a.fournisseurs[j]]
		}
		cp += A[depot][a.fournisseurs[0]]+A[a.nombre_de_fournisseurs-1][usine]
	}
	return(co);
}

/*Optimisation heuristique du cleustering d'un groupe*/
void optimisation(vector<groupe>& groupes, vector<vector<int> > A){
    for (int i=0;i<100000;i++){
        ...
        int cou1=score_groupe(groupes[p],A)+score_groupe(groupes[l],A);
        swap(groupes[p].fournisseurs[a],groupes[l].fournisseurs[b]);
        int cou2=score_groupe(groupes[p],A)+score_groupe(groupes[l],A);
        if (cou1>cou2){
            swap(groupes[p].fournisseurs[a],groupes[l].fournisseurs[b]);
        }	
    }
